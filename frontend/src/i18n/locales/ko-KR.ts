export default {
  ...
  agentEditor: {
    ...
    imageUpload: {
      ...
      sectionDesc: '이미지/오디오 업로드와 VLM·ASR 모델을 설정합니다',
      label: '이미지 업로드',
  ...
  integrations: {
    ...
    api: {
      ...
      principalModeDesc:
        "API 요청이 최종 사용자를 어떻게 식별할지 설정합니다. 이 신원은 대화 Session과 MCP 도구 인증을 사용자별로 구분하며, API Key의 테넌트 관리자 권한은 바뀌지 않습니다.",
      principalScope:
        "최종 사용자별로 대화 Session과 MCP 도구 인증(OAuth)을 구분합니다. API Key는 여전히 테넌트 내 모든 지식베이스에 접근할 수 있으며, API 권한 범위는 외부 사용자별로 나뉘지 않습니다.",
      modeTenant: "테넌트만",
      ...
      requireDirectHeaderDesc:
        "켜면 사용자 ID 헤더가 없는 API Key 요청을 거부하고, 끄면 테넌트 전체로 통일 처리하며 개별 사용자를 구분하지 않습니다.",
      tokenHeader: "토큰 헤더",
      ...
      playgroundTenantModeHint: "테넌트 전용 모드는 외부 사용자 ID를 보내지 않으며 모든 API 세션이 동일한 테넌트 주체에 속합니다.",
      ...
      playgroundStop: "중지",
      playgroundNeedApiKey: "현재 테넌트에 API Key가 없습니다.",
  ...
  imOverview: {
    menuTitle: "연결된 IM",
    pageTitle: "연결된 IM",
    subtitle: "이 테넌트의 에이전트에 연결된 모든 IM 채널 — 활성화/비활성화 전환 또는 해당 에이전트로 이동",
    totalCount: "총 {count}개",
    empty: "이 테넌트에 구성된 IM 봇이 없습니다",
  ...
  tenantMember: {
    ...
    leave: {
      button: "테넌트 떠나기",
      confirmTitle: "이 테넌트에서 나가시겠어요?",
      confirmBody: "이 테넌트의 모든 지식 베이스와 에이전트에 접근할 수 없게 됩니다. 나중에 다시 초대받을 수 있습니다.",
      confirm: "나가기",
      success: "테넌트에서 나갔습니다",
    },
    roleChange: {
