export default {
  ...
  agent: {
    ...
    editor: {
      ...
      skillsCrossLibraryDup: 'Следующие Skills имеют одинаковые имена в нескольких библиотеках Skills, поэтому используемая версия неоднозначна: {names}. При выборе skill между библиотеками дубликаты имён не допускаются; сначала переименуйте skill и повторите попытку.',
...
/example\.(net|org)/`,
  ...
  system: {
    ...
    globalSettings: {
      ...
      keyDescriptions: {
        ...
        tenant: {
          max_owned_per_user:
            'Максимальное число тенантов, которыми может владеть обычный (не супер) пользователь через самостоятельное создание. Читается при каждом создании тенанта и вступает в силу сразу после сохранения. 0 — встроенное значение по умолчанию 10; отрицательное значение полностью снимает ограничение (не рекомендуется для публичных развёртываний).',
          default_storage_quota_gb:
            'Квота хранилища по умолчанию (ГБ) для нового тенанта: векторы, оригиналы, текст, индексы и т.д. Читается только при создании — изменения применяются только к новым тенантам и не перезаписывают существующие. 0 или отрицательное значение — встроенное значение по умолчанию 10 ГБ.',
        },
        asynq: {
          concurrency:
  ...
  embedPublish: {
    ...
    allowMemory: 'Разрешить память',
    allowMemoryDesc: 'При включении анонимные посетители сохраняют контекст в рамках одной сессии; память теряется при очистке данных браузера или новом чате. По умолчанию выключено.',
  ...
  skillsSettings: {
    ...
    libraries: "Библиотеки Skills",

    librariesHint: "Каждый подкаталог верхнего уровня в корне skills — это библиотека Skills. Нажмите, чтобы посмотреть Skills.",

    openLibrary: "Открыть библиотеку",

    skillsUnit: "шт. Skill",

    backToLibraries: "К списку библиотек Skills",

    emptyLibraries: "Нет библиотек Skills",
    libraryNoDesc: "Без описания",
    createLibrary: "Создать библиотеку Skills",
    createLibraryHint: "Создать новый каталог библиотеки для организации Skills",
    libraryNameLabel: "Имя библиотеки",
    libraryNamePlaceholder: "например: custom",
    libraryNameHint: "Начните с буквы или цифры; только буквы, цифры, подчёркивание, точка, дефис; длина 1-64",
    libraryNameRequired: "Введите имя библиотеки",
    libraryDescLabel: "Описание",
    libraryDescPlaceholder: "Необязательно. Кратко опишите назначение этой библиотеки.",
    create: "Создать",
    renameLibrary: "Переименовать",
    rename: "Переименовать",
    libraryNewNameLabel: "Новое имя библиотеки",
    renameSameAsOld: "Новое имя совпадает с текущим",
    actions: "Действия",
    editLibrary: "Изменить",
    save: "Сохранить",
    deleteLibrary: "Удалить",
    delete: "Удалить",
    deleteConfirmBody: "Удалить библиотеку Skills «{name}»? В ней {count} skill(ов). Действие необратимо.",
    ...
    toasts: {
      ...
      createLibraryOk: 'Библиотека Skills создана',
      updateLibraryOk: 'Библиотека Skills обновлена',
      updateLibraryFailed: 'Не удалось обновить библиотеку Skills',
      deleteLibraryOk: 'Библиотека Skills удалена',
      deleteLibraryFailed: 'Не удалось удалить библиотеку Skills',
      createLibraryFailed: 'Не удалось создать библиотеку Skills',
  ...
  agentEditor: {
    ...
    imageUpload: {
      ...
      sectionDesc: 'Настройте загрузку изображений/аудио и соответствующие модели VLM и ASR',
  ...
  integrations: {
    ...
    api: {
      ...
      apiKey: 'API Key арендатора',
      apiKeyDesc: 'API Key остаётся на уровне арендатора; идентификатор пользователя задаётся режимом ниже.',
      principalMode: 'Режим идентификации пользователя',
      principalModeDesc:
        'Настройка того, как API-запросы идентифицируют конечного пользователя. Эта идентичность разделяет сессии диалога и авторизацию MCP-инструментов по пользователям, не сужая доступ API key уровня admin арендатора.',
      principalScope:
        'Идентификация конечного пользователя разделяет сессии диалога и авторизацию MCP-инструментов (OAuth) по пользователям. API key по-прежнему имеет доступ ко всем базам знаний арендатора; область API-прав не делится по внешним пользователям.',
      modeTenant: 'Только арендатор',
      ...
      requireDirectHeaderDesc:
        'Если включено, запросы без заголовка ID пользователя отклоняются; иначе все запросы обрабатываются как единый арендатор без различия конечных пользователей.',
      ...
      secretConfigured: 'Secret настроен; введите новое значение для ротации',
      ...
      playgroundTenantModeHint: 'Режим только арендатора не отправляет внешнего пользователя; все API sessions используют один tenant principal.',
      ...
      playgroundNeedApiKey: 'У этого арендатора нет API key.',
  ...
  tenantMember: {
    ...
    remove: {
      ...
      confirmBody: 'Вы уверены, что хотите удалить {name} из этого тенанта? Доступ будет отозван немедленно.',
      confirm: 'Удалить',
      success: 'Участник удалён',
    },
    leave: {
      button: 'Покинуть тенант',
      confirmTitle: 'Покинуть этот тенант?',
      confirmBody: 'Вы потеряете доступ ко всем базам знаний и агентам в этом тенанте. Позже вас можно будет пригласить снова.',
      confirm: 'Покинуть',
      success: 'Вы покинули тенант',
    },
    roleChange: {
    ...
    errors: {
      ...
      alreadyMember: 'Этот пользователь уже участник тенанта.',
      ...
      noTenant: 'Нет активного тенанта.',
